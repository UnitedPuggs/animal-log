...
<div class="border border-black rounded-lg p-2 w-full">
    <div class="flex">
        {#each days as day}
            ...
            {checkedDays[days.indexOf(day)] ? "bg-sky-900" : "bg-sky-400"} text-white cursor-pointer transition-all duration-200"
            onclick={() => {
                if(!loading)
            ...
            >
                ...
                name="{day}" 
                class="hidden"
                onclick={() => checkedDays[days.indexOf(day)] = !checkedDays[days.indexOf(day)]} id="check-{day}" 
